...
const phraseRegex = new RegExp(/(הרכבת הקלה|מתחם הרכבת|בתקופה זו|תקופה של|תחנת הרכבת|טיילת הרכבת|יכול לשמש|כל התקופה|בן אדם|בעלי חיים|לא עובד|דוד שמש|שימו לב|לאדם|לתקופה|לרכבת|רמי לוי|מוצפת|ברמה|מפרץ חיפה|ברות|משותפת|מראה את|להמליץ|עובד ב|[^ה]מדרגות)/g);
...
const rentRegex = new RegExp(/((?:[1-9][\d,]*[\d])?(?:\s*-\s*[1-9][\d,]*[\d])?\s*(שכר דירה|שכירות|שכר הדירה|ש&quot;ח|ש"ח|שקל|ש״ח|שח|ש''ח|ש'ח|שכה"ד|שכ"ד|שכ״ד|שכה״ד|שכד|כולל ה-כ-ל|שקלים|לחודש|במחיר של|כולל הכלל|לא כולל|כולל הכל|הסכום הוא|מחיר)(?:[^א-ת]|$)\s*-?:?\s*(?:[1-9][\d,]*[\d])?)/g);
...
const stuffRegex = new RegExp("(^|[^א-ת](?:ב?ה?מ?ל?ו?)?)" + 
        "(מיקרוגל|מזרן|מיטה|מכונת כביסה|פינת אוכל|בסיס|קומקום|מגירות|מגירה|מדף|מראה|שידה|שואב אבק|DVD|דיוידי|כורסא|כורסה|כורסאות|מדפים|שידות|מזרון|ספה|ספות|כסאות|כיסאות|כסא|כיסא|מקרר|תנור|כיריים|טוסטר אובן|מייבש|מדיח|מזגן|מאוורר|מסך|טלויזיה|טלוויזיה|שולחנות|שולחן|כורסאות|ארונות|ארון בגדים|ארון|מיטות|ספריה|אופניים|מדפסת|שולחן אוכל|שולחן כתיבה|שידת|מיקרו|מיטת יחיד|מיטה זוגית|ארגז מצעים|שטיח|שטיחים|כורסת|ארונית|רדיאטור|תמי 4|חניה|כבלים|סיבים|ספרייה|הליכון|טלויזיות)"+word_boundry, "g");
